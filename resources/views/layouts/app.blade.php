...
<div class="app-shell">
    ...
    <section class="app-stage">
        <header class="topbar"><button class="mobile-menu"><i data-lucide="menu"></i></button><label class="top-search"><i data-lucide="search"></i><input placeholder="Tìm kiếm task, dự án, người dùng..."><kbd>⌘ K</kbd></label><div class="top-actions"><div class="notification-wrap"><button class="notify" type="button"><i data-lucide="bell"></i><em>3</em></button><div class="notification-menu"><b>Thông báo</b><p>Task “Thiết kế dashboard” sắp đến hạn.</p><p>Bạn được thêm vào dự án Mobile App.</p><p>3 task mới vừa được cập nhật.</p></div></div><button><i data-lucide="message-square"></i></button><div class="user-avatar">{{ mb_strtoupper(mb_substr(auth()->user()->name,0,1)) }}</div><div class="user-meta"><b>{{ auth()->user()->name }}</b><small>{{ auth()->user()->role==='admin'?'Admin':'Thành viên' }}</small></div><form method="post" action="{{ route('logout') }}">@csrf<button title="Đăng xuất"><i data-lucide="chevron-down"></i></button></form></div></header>
