@php($hasFilters = request()->filled('project_id') || request()->filled('priority') || request()->filled('due'))
<form class="task-filter-form {{ $hasFilters ? 'open' : '' }}" method="get" action="{{ $action }}">
    ...
    <div class="filter-panel">
        ...
        <label>Thời hạn<select name="due"><option value="">Tất cả thời hạn</option>@foreach(['today'=>'Hôm nay','tomorrow'=>'Ngày mai','week'=>'Tuần này','none'=>'Không có hạn'] as $value=>$label)<option value="{{ $value }}" @selected(request('due') === $value)>{{ $label }}</option>@endforeach</select></label>
