...
@forelse($projects as $project)
    ...
    <article class="project-card"><div class="card-top"><span class="project-key">{{ $project->key }}</span><div class="icon-text"><span class="badge {{ $project->status }}">{{ ['planning'=>'Lên kế hoạch','active'=>'Đang chạy','on_hold'=>'Tạm dừng','completed'=>'Hoàn thành'][$project->status] ?? $project->status }}</span><div class="action-menu"><button class="action-menu-toggle" type="button" aria-expanded="false" aria-label="Thao tác"><i data-lucide="more-vertical"></i></button><div class="action-menu-popover"><a class="project-edit" href="{{ route('projects.edit',$project) }}"><i data-lucide="pencil"></i>Sửa</a><button type="button" data-delete-action="{{ route('projects.destroy',$project) }}" data-delete-message="Xóa dự án và toàn bộ task bên trong? Hành động không thể hoàn tác."><i data-lucide="trash-2"></i>Xóa</button></div></div></div></div>
        ...
        <footer class="project-card-footer"><div class="project-owner"><span class="avatar">{{ mb_strtoupper(mb_substr($project->owner->name,0,1)) }}</span><span><b>{{ $project->owner->name }}</b><small>Chủ dự án</small></span></div><div class="project-members">@foreach($project->members->take(3) as $member)<span class="avatar" title="{{ $member->name }}">{{ mb_strtoupper(mb_substr($member->name,0,1)) }}</span>@endforeach @if($project->members->count()>3)<span class="avatar more">+{{ $project->members->count()-3 }}</span>@endif</div><time class="{{ $project->due_date?->isPast()&&$project->status!=='completed'?'text-danger':'' }}"><i data-lucide="calendar-days"></i>{{ $project->due_date?->format('d/m/Y') ?? 'Chưa có hạn' }}</time></footer>
    </article>
@empty<div class="label-empty"><i data-lucide="folder"></i><strong>Chưa tìm thấy dự án</strong><span>Thử từ khóa khác hoặc tạo một dự án mới.</span></div>@endforelse
</div>
