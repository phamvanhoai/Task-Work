...
<div class="dashboard-layout"><div class="dashboard-main">
    <section class="metric-grid">
        ...
        <article class="metric-card"><span class="metric-icon purple"><i data-lucide="triangle-alert"></i></span><div><small>Quá hạn</small><strong>{{ $overdueCount }}</strong><em class="{{ $overdueCount?'down':'' }}"><span>Cần xử lý</span></em></div></article>
    </section>
    ...
    @foreach(['todo'=>['Việc cần làm',''], 'in_progress'=>['Đang thực hiện','doing'], 'review'=>['Đang review','review'], 'done'=>['Hoàn thành','done']] as $status=>[$label,$class])
        ...
        @forelse($items as $task)<article class="task-mini"><h4>{{ $task->title }}</h4><span class="tag {{ $status==='done'?'pink':'green' }}">{{ $task->project->name }}</span><div class="task-mini-footer"><span class="mini-avatar">{{ mb_substr(auth()->user()->name,0,1) }}</span><span class="icon-text"><i data-lucide="calendar-days"></i>{{ $task->due_date?->format('d/m')??'—' }}</span></div></article>@empty<p class="kanban-empty">Không có task</p>@endforelse
    ...
    <section class="dash-side-card"><div class="card-head"><h3>Hoạt động gần đây</h3></div>@forelse($recentNotifications as $notification)<div class="activity-row"><span class="avatar">{{ mb_substr($notification->data['title']??'T',0,1) }}</span><p>{{ $notification->data['title']??'Thông báo' }}<small>{{ $notification->created_at->diffForHumans() }}</small></p></div>@empty<p class="empty">Chưa có hoạt động.</p>@endforelse</section>
